function TernaryPlot_color_optional(data)

%Calculating most likely cluster composition for each pair combination
...
sample_color_2 = [1 0 0].*Protein_1./100 + [0 0 1].*Protein_2./100 + [0 1 0].*Protein_3./100 ;
 ...
 % Colors defined by most likely cluster composition
 C = ones(size(sample_pts, 1), 1).*sample_pts(:, 1);
 s = [pCSK_PAG,pCSK_TRAF3,pTRAF_PAG,pall]; s=s*170;
...
scatter(sample_pts(:, 1), sample_pts(:, 2),s, sample_color_2, 'filled','MarkerFaceAlpha',.2);pbaspect([1 1 1])
