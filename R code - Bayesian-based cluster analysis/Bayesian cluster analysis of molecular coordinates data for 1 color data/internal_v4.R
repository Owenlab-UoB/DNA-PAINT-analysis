...
get_max<-function(r,mat,av){
  ...
  if (num_mat>3){
    ...
    for (i in 1:num_l){
      ...
      if(length(D)-1>(av+1) | length(D)-1>3){
        ########### see if max
        MM=max(D)
        if(MM==L[i]){
          #the "plateau" case
...
###################################################################################
  ...
  for (r in rseq){
    ...
    av_point_per_nm=num_l/(3000*3000)
    ...
    if (NEW_R==TRUE){ 
      ...
      mat[,3]=Lr-(L_mean+2*sigma_TBU) # 95% confidence interval
...
molsPerClusterii <- function(labels,i,D1,D2){
  s=length(labels); 
  count_channel1=0
  count_channel2=0
  n=as.numeric(i)
  ...
  return(c(count_channel1,count_channel2))
}
