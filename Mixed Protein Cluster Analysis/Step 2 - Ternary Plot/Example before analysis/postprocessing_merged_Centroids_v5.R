...
sapply(foldernames, function(expname){
    ...
    res=lapply(dirnames, function(dirname){
        ...
        if (skeleton){
            ...
            file.copy(file.path(paste(foldername, "/ClusterCentroids_merged_v1.txt", sep="")), file.path(paste(nfoldername, "/data_channel1.txt", sep="")))
        }
        data=read.csv(file.path(paste(nfoldername, "/ClusterCentroids_merged_v1.txt", sep="")))
        data1=read.csv(file.path(paste(nfoldername, "/ClusterCentroids_Ch2_v1.txt", sep="")))
        data2=read.csv(file.path(paste(nfoldername, "/ClusterCentroids_Ch3_v1.txt", sep="")))
        data3=read.csv(file.path(paste(nfoldername, "/ClusterCentroids_Ch4_v1.txt", sep="")))
        D1=dim(data1)[1]
        D2=dim(data2)[1]
        D3=dim(data3)[1]
        D4=dim(data)[1]
        ...
        if (skeleton){
            file.copy(file.path(paste(foldername, "/r_vs_thresh_ClusterCentroids_merged_v1.txt", sep="")), file.path(paste(nfoldername, "/r_vs_thresh_ClusterCentroids_merged_v1.txt", sep="")))
        }
        r = read.csv(file.path(paste(nfoldername, "/r_vs_thresh_ClusterCentroids_merged_v1.txt",sep="")), header=FALSE, sep="\t")
        ...
        bestcs=cs[best[2]]
        bestthr=thr[best[1]]
        bfile=file.path(paste(foldername, "/labelsf/clusterscale", bestcs, " thresh", bestthr, "labels.txt", sep=""))
        nbfile=bfile
        if (skeleton){
            dir.create(paste(nfoldername, "/labelsf", sep=""))    
            nbfile=file.path(paste(nfoldername, "/labelsf/clusterscale", bestcs, " thresh", bestthr, "labels.txt", sep=""))
        ...
        for (k in 1:length(cluster_list)){
          ...
          nmol_list=rbind(nmol_list, molsPerClusterii(labelsbest,cluster_list[k],D1,D2,D3))
        ...
        ##saving the file
        write.csv(LIST,file = paste(nfoldername, "/list_Centroids_merged_v1.csv", sep=""), sep=",")
        ...
        ##Some summaries
        wfile=file.path(paste(nfoldername, "/summary_Centroids_merged_v1.txt", sep=""))
        ...
        if (makeplot){
            if (!superplot){
                    pdf(file.path(paste(nfoldername, "/plot_channel_Centroids_merged_v1g.pdf", sep="")))
                    axes=TRUE
                    cex=1
                }           
            if (dim(data)[2]==4){
    ...
    pdf(file.path(paste(nexpname, "/nmols_channel_Centroids_merged_v1.pdf", sep="")))
    plot(h, xlab="Number of molecules", ylab="Number of clusters", main="")
    dev.off()
    f=file.path(paste(nexpname, "/nmols_channel_Centroids_merged_v1.txt", sep="")); cat(nmols, file=f, sep=","); cat("\n", file=f, append=TRUE)
    ...
    pdf(file.path(paste(nexpname, "/radii_channel_Centroids_merged_v1.pdf", sep="")))
    plot(h, xlab="Cluster radius", ylab="Number of clusters", main="")
    dev.off()
    f=file.path(paste(nexpname, "/radii_channel_Centroids_merged_v1.txt", sep="")); cat(radii, file=f, sep=","); cat("\n", file=f, append=TRUE)
    ...
    pdf(file.path(paste(nexpname, "/nclusters_channel_Centroids_merged_v1.pdf", sep="")))
    plot(h, xlab="Number of clusters", ylab="Number of regions", main="")
    dev.off()
    f=file.path(paste(nexpname, "/nclusters_channel_Centroids_merged_v1.txt", sep="")); cat(nclusters, file=f, sep=","); cat("\n", file=f, append=TRUE)
    ...
    pdf(file.path(paste(nexpname, "/pclustered_channel_Centroids_merged_v1.pdf", sep="")))
    plot(h, xlab="Percentage clustered", ylab="Number of regions", main="")
    dev.off()
    f=file.path(paste(nexpname, "/pclustered_channel_Centroids_merged_v1.txt", sep="")); cat(pclustered, file=f, sep=","); cat("\n", file=f, append=TRUE)   
    ...
    pdf(file.path(paste(nexpname, "/totalmols_channel_Centroids_merged_v1.pdf", sep="")))
    plot(h, xlab="Total Mols per ROI", ylab="Number of regions", main="")
    dev.off()
    f=file.path(paste(nexpname, "/totalmols_channel_Centroids_merged_v1.txt", sep="")); cat(totalmols, file=f, sep=","); cat("\n", file=f, append=TRUE)   
    ...
    pdf(file.path(paste(nexpname, "/reldensity_channel_Centroids_merged_v1.pdf", sep="")))
    plot(h, xlab="Total Mols per ROI", ylab="Number of regions", main="")
    dev.off()
    f=file.path(paste(nexpname, "/reldensity_channel_Centroids_merged_v1.txt", sep="")); cat(reldensity, file=f, sep=","); cat("\n", file=f, append=TRUE)
