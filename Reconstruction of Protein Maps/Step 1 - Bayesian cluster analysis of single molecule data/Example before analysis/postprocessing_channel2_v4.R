...
sapply(foldernames, function(expname){
    ...
    res=lapply(dirnames, function(dirname){
        ...
        if (skeleton){
            ...
            file.copy(file.path(paste(foldername, "/data_channel2.txt", sep="")), file.path(paste(nfoldername, "/data_channel1.txt", sep="")))
        }
        data=read.csv(file.path(paste(nfoldername, "/data_channel2.txt", sep="")))
        
        pts = data[,1:2]; sds = data[,3];
        if (skeleton){
            file.copy(file.path(paste(foldername, "/r_vs_thresh_channel2.txt", sep="")), file.path(paste(nfoldername, "/r_vs_thresh_channel1.txt", sep="")))
        }
        r = read.csv(file.path(paste(nfoldername, "/r_vs_thresh_channel2.txt",sep="")), header=FALSE, sep="\t")
        ...
        ##saving the file
        write.csv(LIST,file = paste(nfoldername, "/list_channel2.csv", sep=""), sep=",")
        
        #################################################################################
        
        ##Some summaries
        wfile=file.path(paste(nfoldername, "/summary_channel2.txt", sep=""))
        ...
        if (makeplot){
            if (!superplot){
                    pdf(file.path(paste(nfoldername, "/plot_channel2.pdf", sep="")))
                    axes=TRUE
                    cex=1
                }           
            if (dim(data)[2]==4){
    ...
    pdf(file.path(paste(nexpname, "/nmols_channel2.pdf", sep="")))
    plot(h, xlab="Number of molecules", ylab="Number of clusters", main="")
    dev.off()
    f=file.path(paste(nexpname, "/nmols_channel2.txt", sep="")); cat(nmols, file=f, sep=","); cat("\n", file=f, append=TRUE)
    ...
    pdf(file.path(paste(nexpname, "/radii_channel2.pdf", sep="")))
    plot(h, xlab="Cluster radius", ylab="Number of clusters", main="")
    dev.off()
    f=file.path(paste(nexpname, "/radii_channel2.txt", sep="")); cat(radii, file=f, sep=","); cat("\n", file=f, append=TRUE)
    ...
    pdf(file.path(paste(nexpname, "/nclusters_channel2.pdf", sep="")))
    plot(h, xlab="Number of clusters", ylab="Number of regions", main="")
    dev.off()
    f=file.path(paste(nexpname, "/nclusters_channel2.txt", sep="")); cat(nclusters, file=f, sep=","); cat("\n", file=f, append=TRUE)
    ...
    pdf(file.path(paste(nexpname, "/pclustered_channel2.pdf", sep="")))
    plot(h, xlab="Percentage clustered", ylab="Number of regions", main="")
    dev.off()
    f=file.path(paste(nexpname, "/pclustered_channel2.txt", sep="")); cat(pclustered, file=f, sep=","); cat("\n", file=f, append=TRUE)   
    ...
    pdf(file.path(paste(nexpname, "/totalmols_channel2.pdf", sep="")))
    plot(h, xlab="Total Mols per ROI", ylab="Number of regions", main="")
    dev.off()
    f=file.path(paste(nexpname, "/totalmols_channel2.txt", sep="")); cat(totalmols, file=f, sep=","); cat("\n", file=f, append=TRUE)   
    ...
    pdf(file.path(paste(nexpname, "/reldensity_channel2.pdf", sep="")))
    plot(h, xlab="Total Mols per ROI", ylab="Number of regions", main="")
    dev.off()
    f=file.path(paste(nexpname, "/reldensity_channel2.txt", sep="")); cat(reldensity, file=f, sep=","); cat("\n", file=f, append=TRUE)
